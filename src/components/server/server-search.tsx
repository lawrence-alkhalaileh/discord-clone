...
import UserOperatingSystem from "./user-operating-system";
  ...
  useEffect(() => {
    const down = (e: KeyboardEvent) => {
      if (e.key == "q" && (e.ctrlKey || e.metaKey)) {
        console.log("search short cut pressed");
        e.preventDefault();
        setOpen((open) => !open);
      }
    };
  ...
  return (
    <>
      ...
      >
        ...
        h-6 w-15 select-none items-center gap-1 rounded
        border bg-muted px-1.5 font-mono text-[10px]
        font-medium text-muted-foreground ml-auto"
        >
          <span className="text-[9px] m-auto">
            <UserOperatingSystem />
          </span>
        </kbd>
      </button>
      <CommandDialog open={open} onOpenChange={setOpen}>
        <CommandInput placeholder="Search all channels all members" />
        <CommandList>
